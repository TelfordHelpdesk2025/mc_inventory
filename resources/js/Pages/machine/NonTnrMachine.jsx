...
const FIELD_GROUPS = {
  general: {
    ...
    fields: [
      ...
      { name: "machine_name", label: "Machine Name", type: "select", optionsKey: "machine_name" },
      { name: "location", label: "Location", type: "select", optionsKey: "location" },
      { name: "serial", label: "Serial" },
      { name: "pmnt_no", label: "PMNT No" },
      { name: "frequency", label: "Frequency", type: "select", optionsKey: "frequency" },
      { name: "model", label: "Model", type: "select", optionsKey: "model" },
      { name: "cn_no", label: "CN No" },
      { name: "manufacturer", label: "Manufacturer", type: "select", optionsKey: "manufacturer" },
      { name: "remarks", label: "Remarks", type: "select", optionsKey: "remarks" },
...
/* ======================================================
   MAIN COMPONENT
====================================================== */
export default function NonTnrMachine({
  tableData,
  tableFilters,
  emp_data,
  existingMachine,
...
const handleDelete = (id) => {
  if (confirm("Are you sure you want to delete this machine?")) {
    router.delete(route("non.tnr.machine.delete", id), {
      preserveScroll: true,
      onSuccess: () => {
  ...
  const submitEdit = e => {
    e.preventDefault();
    router.put(route("non.tnr.machine.update", selectedMachine.id), formData, {
  ...
  const submitAdd = e => {
    e.preventDefault();
    router.post(route("non.tnr.machine.store"), formData, {
      onSuccess: () => window.location.reload(),
    });
  };
...
const getRemarksClass = (remarks) => {
    if (!remarks) return "text-slate-500 border-slate-400 bg-slate-100";

    return (
        STATUS_STYLES[remarks.trim().toLowerCase()] ||
  ...
  remarks: (
    <span
        className={`px-2 py-1 text-xs font-semibold border rounded-md transition-all duration-200 ${getRemarksClass(
            item.remarks
        )}`}
    >
        {item.remarks?.trim() || "Waiting to Update..."}
    </span>
),
  ...
  ownership: (
    <span
      className={`px-2 py-1 text-xs font-semibold border rounded-md ${
        item.ownership &&
        item.ownership.trim().toLowerCase() === "consigned"
          ? "text-blue-500 border-blue-500 bg-blue-100 hover:bg-blue-700 hover:text-white"
          : item.ownership &&
            item.ownership.trim().toLowerCase() === "owned"
    ...
    >
      {item.ownership?.trim() || "Waiting to Update..."}
    </span>
  ),
  ...
  // 🔸 Action buttons
    action: (
      ...
      <button
        ...
        className="px-3 py-1 bg-red-500 text-white rounded border border-red-900 text-sm hover:bg-red-600 flex items-center gap-1"
      >
  ...
  return (
    <AuthenticatedLayout>
      <Head title="Non Tnr Machine" />

      <div className="flex justify-between mb-4">
        <h1 className="text-2xl font-bold"><ScannerTwoToneIcon style={{fontSize: 45, marginBottom: 10}}/> Non Tnr Machine</h1>
        <button
          onClick={openAddModal}
          className="bg-green-500 text-white px-3 rounded hover:bg-green-600"
        >
          <PlusOutlined style={{fontSize: 18, fontWeight: "bold"}} /> NEW Non-Tnr Machine
        </button>
      </div>
       ...
       <DataTable
                ...
                routeName={route("non.tnr.machine.index")}
...
<Drawer
  ...
  onClose={closeAll}
  size={795}  // pwede 70vw, 80vw, 100vw
...
>
  {selectedMachine && (
    <div className="flex h-full">
      ...
      {/* CONTENT */}
      <div className="flex-1 p-6 space-y-4 overflow-y-auto text-2xl">
    ...
    <div className="flex items-center gap-2 font-semibold">
      {isEditModalOpen ? (
        ...
        <>
          ...
          <span className="text-emerald-600">New Non Tnr Machine</span>
        </>
      )}
    </div>
  }
