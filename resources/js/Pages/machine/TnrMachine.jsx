...
const FIELD_GROUPS = {
  ...
  location: {
    ...
    fields: [
      ...
      { name: "ownership", label: "Ownership", type: "select", optionsKey: "ownership" },
    ],
  },
...
/* ======================================================
   MAIN COMPONENT
====================================================== */
export default function TnrMachine({
  tableData,
  tableFilters,
  emp_data,
  existingMachine,
...
const handleDelete = (id) => {
  if (confirm("Are you sure you want to delete this machine?")) {
    router.delete(route("tnr.machine.delete", id), {
      preserveScroll: true,
      onSuccess: () => {
  ...
  const submitEdit = e => {
    e.preventDefault();
    router.post(route("tnr.machine.update", selectedMachine.id), formData, {
  ...
  const submitAdd = e => {
    e.preventDefault();
    router.post(route("tnr.machine.store"), formData, {
      onSuccess: () => window.location.reload(),
    });
  };
...
const getStatusClass = (status) => {
    ...
    <span
      className={`px-2 py-1 text-xs font-semibold border rounded-md ${
        ...
        item.ownership.trim().toLowerCase() === "consigned"
          ...
          : item.ownership &&
            item.ownership.trim().toLowerCase() === "tspi-owned"
  ...
  return (
    <AuthenticatedLayout>
      <Head title="TNR Machine" />

      <div className="flex justify-between mb-4">
        <h1 className="text-2xl font-bold"><ScannerTwoToneIcon style={{fontSize: 45, marginBottom: 10}}/> TNR Machine</h1>
        <button
       ...
       <DataTable
                ...
                routeName={route("tnr.machine.index")}
    ...
    <div className="flex items-center gap-2 font-semibold">
      {isEditModalOpen ? (
        ...
        <>
          ...
          <span className="text-emerald-600">New Tnr Machine</span>
        </>
      )}
    </div>
  }
