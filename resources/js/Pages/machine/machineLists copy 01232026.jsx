...
const machineTypes = [
  ...
  <div key={field}>
    <label className="text-sm font-medium capitalize">
     ...
     {field === "machine_feed_type" ? (
  <Select
    options={machineFeedType.map((type) => ({ label: type, value: type }))}
    value={
      formData.machine_feed_type
        ? { label: formData.machine_feed_type, value: formData.machine_feed_type }
        : null
    }
    onChange={(selected) =>
      setFormData((prev) => ({ ...prev, machine_feed_type: selected.value }))
    }
  />
) : field === "machine_type" ? (
  <Select
    options={machineTypes.map((type) => ({ label: type, value: type }))}
    value={
      formData.machine_type
        ? { label: formData.machine_type, value: formData.machine_type }
        : null
    }
    onChange={(selected) =>
      setFormData((prev) => ({ ...prev, machine_type: selected.value }))
    }
  />
) : (
  <input
